...
def main():
    ...
    import os
    if os.path.exists('revealresults_llmpre.csv'):
        try:
            with open('revealresults_llmpre.csv', 'r') as f:
                reader = csv.reader(f)
                header = next(reader, None)
                for r in reader:
                    if len(r) >= 2:
                        try:
                            prediction_ls.append(int(r[0]))
                            ground_truth.append(int(r[1]))
                        except ValueError:
                            pass
            start_idx = len(prediction_ls)
            print(f"Found existing results. Resuming from index {start_idx} / {len(data)}")
        except Exception as e:
            print(f"Could not read existing results: {e}. Starting from 0.")
            prediction_ls = []
            ground_truth = []
            start_idx = 0
    ...
    for i in range(start_idx, len(data)):
        ...
        if 'func' in row:
            ...
            prompt = format(inputCode) + templates[1] + templates[2] + format(inputnode) + templates[3] + format(inputedge) + templates[4] + format(inputex)
            try:
            ...
            with open('revealresults_llmpre.csv', 'a', newline='') as f:
                writer = csv.writer(f)
